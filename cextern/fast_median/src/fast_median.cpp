...
// T: float or double.
// x image dimensions, width x pixels.
// hx: window RADIUS
// blockhint: preferred block size, 0 = pick automatically.
// in: input image.
// out: output image.
//
// Total window size will be (2*hx+1) pixels.
template <typename T>
...
template void median_filter_2d<float>(int nx, int ny, int hx, int hy, int blockhint, const float *in, float *out);
template void median_filter_2d<double>(int nx, int ny, int hx, int hy, int blockhint, const double *in, double *out);

template void median_filter_1d<float>(int nx, int hx, int blockhint, const float *in, float *out);
template void median_filter_1d<double>(int nx, int hx, int blockhint, const double *in, double *out);


/*
 *   Interface to Python:
 *   Follow interface of ndimage.median_filter where size=(dx,dy) is box size (not radius).
 */
void median_filter_2d_float(int nx, int ny, int box_x, int box_y, int blockhint, const float *in, float *out)
{
    int hx = (box_x-1)/2; // radius from box size
    int hy = (box_y-1)/2;
    return median_filter_2d<float>(nx, ny, hx, hy, blockhint, in, out);
}
...
void median_filter_2d_double(int nx, int ny, int box_x, int box_y, int blockhint, const double *in, double *out)
{
    int hx = (box_x-1)/2; // radius from box size
    int hy = (box_y-1)/2;
    return median_filter_2d<double>(nx, ny, hx, hy, blockhint, in, out);
}
...
{
    int hx = (box_x-1)/2; // radius from box size
    return median_filter_1d<float>(nx, hx, blockhint, in, out);
}
