...
def test_detect_shifts_noshift(make_fits):
    ...
    image_ori = loadImage(rpath)
    shift_columns, which_shifts, corrs, image_fixed = pixshifts.detect_shifts(in_image=ipath, ref_image=rpath)

    assert (shift_columns == 0).all()
    assert (image_fixed._data == image_ori._data).all()


def test_detect_shifts(make_fits):
