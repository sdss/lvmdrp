...
class TraceMask(FiberRows):
    ...
    def smoothTraceDist(self, start_slice, poly_cross=[4, 1, 4], poly_disp=8, bound=[350, 2000]):
        ...
        (init_dist, init_mask) = self.getFiberDist(start_slice) # get initial fiber distances
        change_dist[:, start_slice] = init_dist # insert the initial distance into array
        first = numpy.arange(start_slice, -1, -1) 
        x = numpy.arange(self._fibers-1)-self._fibers/2.0 # define the cross-disperion coordinate system
        fit_par = numpy.zeros((len(poly_cross), self._data.shape[1]), dtype=numpy.float32) #empty array to store the poly fit parameters in cross-dispersion direction
   ...
   #             fit = numpy.polyfit(x[select_good][select], change[select_good][select],  len(poly_cross)-1) # fit the relative change in the fiber distance with a polynomial of given order
        ...
        #        select = numpy.abs(res)<=3*numpy.std(res)
         #       fit = numpy.polyfit(x[select_good][select], change[select_good][select],  len(poly_cross)-1) # fit the relative change in the fiber distance with a polynomial of given order
            ...
            if i==-3930:
               ...
               print good_mask
               #pylab.plot(change, '-k')
               pylab.plot(x[select_good][select], change_dist[select_good, i][select], 'ok')
     ...
     #   print wave[select_wave]
        ...
        for j in range(len(poly_cross)):
            fit = numpy.polyfit(wave[select_wave]-fit_par.shape[1]/2.0, fit_par[j, select_wave], poly_cross[len(poly_cross)-j-1]) # fit polynomial along dispersion axis
            res = fit_par[j, select_wave]-numpy.polyval(fit, wave[select_wave]-fit_par.shape[1]/2.0)
            rms = numpy.std(res)
 ...
 #           pylab.plot(wave[select_wave], res, '-k')
   #         pylab.plot(wave[select_wave], fit_par[j, select_wave], 'ok')
    ...
    #    pylab.show()
        ...
        offset_mean = numpy.median(self._data[:, select_wave]-new_trace[:, select_wave], axis=0)  # computes that absolut trace position between the initially measured and estimated trace to compute the zero-point
        offset_rms = numpy.std(self._data[:, select_wave]-new_trace[:, select_wave], axis=0)  # compute the rms scatter of the measured positions for each dispersion column 
