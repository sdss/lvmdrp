...
class Image(Header):
	...
	def calibrateSDSS(self, fieldPhot, subtractSky=True):
		...
		factor = old_div((10**(0.4*(aa+(kk*airmass)))),exptime)
		...
		if subtractSky:
			...
			error = numpy.sqrt(old_div((calibratedImage+sky),gain) + dark_var )
		else:
			error = numpy.sqrt(old_div((calibratedImage),gain) + dark_var )
...
#        return new_image
	...
	def fitPoly(self, axis='y', order=4, plot=-1):
		...
		for i in range(slices):
			...
			if self._mask  is not None:
				...
				fit_par[:, i] = numpy.polyfit(x[self._mask[:, i]][select], self._data[self._mask[:, i], i][select], order) #refit polynomial with clipped data
				if plot==i:
					pylab.plot(x, self._data[:, i], '-b')
					pylab.plot(x[self._mask[:, i]][select], self._data[self._mask[:, i], i][select], 'ok')
					max = numpy.max(self._data[self._mask[:, i], i][select])
			else:
		...
		new_img = Image(data=fit_result, error=self._error, header = self._header,  mask=self._mask)
		return new_img
	...
	def maskFiberTraces(self, TraceMask, aperture=3, parallel='auto'):
		...
		self._mask = numpy.ones(self._dim, dtype='bool')
		if parallel=='auto':
			cpus = cpu_count()
		else:
			cpus = int(parallel)
		...
		for i in range(self._dim[1]):
			if cpus>1:
				self._mask[mask_pixels[i].get(), i] = False
			else:
	...
	def centreBary(self, guess_x, guess_y, box_size, exponent=4):
		...
		if self._mask is not None:
			...
			cent_y = old_div(numpy.sum((idx_y*region**exponent)[good_pix]),numpy.sum(region[good_pix]**exponent))+1
		else:
			cent_x = old_div(numpy.sum((idx_x*region**exponent)),numpy.sum(region**exponent))+1
			cent_y = old_div(numpy.sum((idx_y*region**exponent)),numpy.sum(region**exponent))+1
		return cent_x, cent_y
	...
	def extractAper(self, cent_x, cent_y, aperture, kmax=1000, correct_masked=False, ignore_mask=True):
		...
		def integrate_aperture(image, xc, yc, r_ap, kmax, error=None, mask=None, ignore_mask=ignore_mask, correct_masked=correct_masked):
			...
			correction_factor=old_div(expected_area,total_area)
			coverage = old_div(total_area,expected_area)
			if correct_masked:
	...
	def extractApertures(self, posTab,  ref_pixel_x, ref_pixel_y, arc_scale, angle=0, offset_arc_x=0.0, offset_arc_y=0.0 ):
		...
		offset_x=old_div((offset_arc_x*numpy.cos(float(angle)/180.0*numpy.pi)-offset_arc_y*numpy.sin(float(angle)/180.0*numpy.pi)),arc_scale)
		offset_y=old_div((offset_arc_x*numpy.sin(float(angle)/180.0*numpy.pi)+offset_arc_y*numpy.cos(float(angle)/180.0*numpy.pi)),arc_scale)
