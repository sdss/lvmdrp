...
class Cube(Header, PositionTable):
	...
	def collapseCube(self, mode='mean', start_wave=None, end_wave=None):
		...
		if mode=='mean':
			...
			mask= image==0
		img = Image(data=image, mask=mask)
	...
	def glueCubeSets(self, cube1, cube2, rescale_region=[], merge_wave=0.0, mergeHdr=True):
		...
		if merge_wave==0.0:
			if (disp1) == (disp2) and (disp1*numpy.rint(old_div(((wave2[0]-wave1[0])),(disp1)))).astype(numpy.float32)==(wave2[0]-wave1[0]).astype(numpy.float32):
		...
		for i in range(self._data.shape[2]):
			for j in range(self._data.shape[1]):
				if rescale_region!=[] and slice1._data[j+crpix2_1-crpix2, i+crpix1_1-crpix1]!=0:
					ratio = old_div(slice2._data[j+crpix2_2-crpix2, i+crpix1_2-crpix1],slice1._data[j+crpix2_1-crpix2, i+crpix1_1-crpix1])
				else:
					ratio=1
				if ratio<0:
					ratio=0
...
def loadCube(infile, extension_data=None, extension_mask=None,  extension_error=None, extension_cover=None):

	cube = Cube()
	cube.loadFitsData(infile, extension_data=extension_data, extension_mask=extension_mask,  extension_error=extension_error, extension_cover=extension_cover)
