...
REDUCTION_STEPS = {
    ...
    "arc": ["pre", "calib", "cosmic", "stray", "extract", "wave", "pixtable", "waveres"],
...
def define_inout_paths(params, metadata, calib_metadata, prev_step, curr_step, path):
    ...
    curr_io_param_names = STEPS_FILE_PARS[curr_step]["in"] + STEPS_FILE_PARS[curr_step]["out"]
    # define previous and current step params
    curr_params = params[curr_step]
    # extract those params corresponding to in/out files
    curr_paths = {key: value for key, value in curr_params.items() if key in curr_io_param_names}
    # depending on the reduction type and step, set path params: path, label, kind
    if curr_step == "pre":
        curr_paths["in_image"] = curr_paths["in_image"].format(path=path, mjd=metadata.mjd, label=metadata.label)
        curr_paths["out_image"] = curr_paths["out_image"].format(path=path, label=metadata.label, kind=curr_step)
    elif curr_step == "calib":
        curr_paths["in_image"] = curr_paths["in_image"].format(path=path, label=metadata.label, kind="pre")
        curr_paths["bias"] = calib_metadata["bias"].path if calib_metadata["bias"] is not None else None
        curr_paths["dark"] = calib_metadata["dark"].path if calib_metadata["dark"] is not None else None
        curr_paths["flat"] = calib_metadata["flat"].path if calib_metadata["flat"] is not None else None
        curr_paths["out_image"] = curr_paths["out_image"].format(path, label=metadata.label, kind=curr_step)
    elif curr_step == "peaks":
        curr_paths["image"] = curr_paths["image"].format(path=path, label=metadata.label, kind=prev_step)
        curr_paths["out_peaks_file"] = curr_paths["out_peaks_file"].format(path=path, label=metadata.label, kind=curr_step)
    elif curr_step == "trace":
        curr_paths["image"] = curr_paths["image"].format(path=path, label=metadata.label, kind=prev_step)
        curr_paths["peaks_file"] = curr_paths["peaks_file"].format(path=path, label=metadata.label, kind="peaks")
        curr_paths["trace_out"] = curr_paths["trace_out"].format(path=path, label=metadata.label, kind=curr_step)
    elif curr_step == "stray":
        curr_paths["image"] = curr_paths["image"].format(path=path, label=metadata.label, kind=prev_step)
        curr_paths["trace"] = curr_paths["trace"].format(path=path, label=metadata.label, kind="trace")
        curr_paths["stray_image"] = curr_paths["stray_image"].format(path=path, label=metadata.label, kind="back")
        curr_paths["clean_image"] = curr_paths["clean_image"].format(path=path, label=metadata.label, kind=curr_step)
    elif curr_step == "fwhm":
        curr_paths["image"] = curr_paths["image"].format(path=path, label=metadata.label, kind=prev_step)
        curr_paths["trace"] = curr_paths["trace"].format(path=path, label=metadata.label, kind="trace")
        curr_paths["fwhm_out"] = curr_paths["fwhm_out"].format(path=path, label=metadata.label, kind=curr_step)
    elif curr_step == "extract":
        curr_paths["image"] = curr_paths["image"].format(path=path, label=metadata.label, kind=prev_step)
        curr_paths["trace"] = curr_paths["trace"].format(path=path, label=metadata.label, kind="trace")
        curr_paths["out_rss"] = curr_paths["out_rss"].format(path=path, label=metadata.label, kind=curr_step)
    elif curr_step == "wave":
        curr_paths["arc_rss"] = curr_paths["arc_rss"].format(path=path, label=metadata.label, kind=prev_step)
        curr_paths["disp_rss"] = curr_paths["disp_rss"].format(path=path, label=metadata.label, kind="disp")
        curr_paths["res_rss"] = curr_paths["res_rss"].format(path=path, label=metadata.label, kind="res")
        curr_paths["ref_line_file"] = calib_metadata["pix2wave"].path
    elif curr_step == "pixtable":
        curr_paths["rss_in"] = curr_paths["rss_in"].format(path=path, label=metadata.label, kind=prev_step)
        curr_paths["rss_out"] = curr_paths["rss_out"].format(path=path, label=metadata.label, kind=curr_step)
        curr_paths["arc_wave"] = curr_paths["arc_wave"].format(path=path, label=metadata.label, kind="disp")
        curr_paths["arc_fwhm"] = curr_paths["arc_fwhm"].format(path=path, label=metadata.label, kind="res")
    elif curr_step == "waveres":
        curr_paths["rss_in"] = curr_paths["rss_in"].format(path=path, label=metadata.label, kind=prev_step)
        curr_paths["rss_out"] = curr_paths["rss_out"].format(path=path, label=metadata.label, kind="rss")
...
def setup_reduction(config, metadata):
    ...
    settings.pix2wave_map = os.path.join(config.LVM_DRP_CONFIG_PATH, config.PIX2WAVE_MAPS.__dict__[metadata.ccd])
    settings.lamps = [name for name in metadata.__data__ if name in LAMP_NAMES and metadata.__data__[name]]
    ...
    settings.output_path = config.LVM_SPECTRO_CALIB_PATH if settings.type in CALIBRATION_TYPES else config.LVM_SPECTRO_REDUX_PATH
    ...
    for step in REDUCTION_STEPS[settings.type]:
        ...
        paths = define_inout_paths(
            ...
            prev_step=last_step, curr_step=step,
...
def run_reduction_calib(config, metadata, calib_metadata, settings):
    ...
    calib_image.writeFitsData(settings.output_path.format(label=metadata.label, kind="calib"))
...
def run_reduction_block(config, metadata, calib_metadata, settings):
    ...
    target_frame_path = settings.output_path.format(
...
def run_reduction_arc(config, metadata, calib_metadata, settings):
    metadata = run_reduction_block(config, metadata=metadata, calib_metadata=calib_metadata, settings=settings)
    target_frame_path = settings.output_path.format(
        label=metadata.label,
        kind="{kind}"
    )
...
def run_reduction_object(config, metadata, calib_metadata, settings):
    ...
    _, flags = rssMethod.createPixTable_drp(
        ...
        arc_wave=settings.output_path.format(kind="disp"), arc_fwhm=settings.output_path.format(kind="res"), cropping=''
    )
    metadata.flags += flags
    _, flags = rssMethod.resampleWave_drp(
