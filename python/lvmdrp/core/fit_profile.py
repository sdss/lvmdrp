            ...
            if self._profile_type[n]=='TemplateScale':
                if self._fixed[n]['scale']!=1:
                    try:
                        ...
                        line = self._parameters[n]['scale'].split(':')
                        if len(line)==1:
                            self._parameters_err[n]['scale'] = self._parameters_err[line[0]]['scale']
                        else:
                            self._parameters_err[n]['scale'] = self._parameters_err[line[0]]['scale']*float(line[1])
            if self._profile_type[n]=='Gauss':
                if self._fixed[n]['flux']!=1:
                    try:
                        ...
                        line = self._parameters[n]['flux'].split(':')
                        if len(line)==1:
                            self._parameters_err[n]['flux'] = self._parameters_err[line[0]]['flux']
                        else:
                            self._parameters_err[n]['flux'] = self._parameters_err[line[0]]['flux']*float(line[1])
                if self._fixed[n]['vel']!=1:
                    try:
                        float(self._parameters[n]['vel'])
                        self._parameters_err[n]['vel']=0
                    except ValueError:
                        self._parameters_err[n]['vel'] = self._parameters_err[self._parameters[n]['vel']]['vel']
                if  self._fixed[n]['disp']!=1:
                    try:
                        float(self._parameters[n]['disp'])
                        self._parameters_err[n]['disp']=0
                    except ValueError:
                        self._parameters_err[n]['disp'] = self._parameters_err[self._parameters[n]['disp']]['disp']
        ...
        for n in self._names:
            if self._profile_type[n]=='TemplateScale':
                if self._fixed[n]['scale']==1:
                    ...
                    scale_guess = numpy.sum(y[select_match])/numpy.sum(self._template_spec[n]._data[select_match])*0.95
            ...
            if self._profile_type[n]=='Gauss':
                ...
                if self._fixed[n]['disp']==1:
                    ...
                    try:
                        ...
                        width = numpy.sqrt(numpy.sum((temp_y[select]*(x[select]-wave_z)**2))/(numpy.sum(temp_y[select])))
                        if width>self._spec_res.getRes(wave_z) and numpy.isnan(width)==False:
                            disp = numpy.sqrt(width**2-self._spec_res.getRes(wave_z)**2)/(wave_z)*300000.0
    ...
    def writeTablePar(self,outfile):
        ...
        for k1 in self._parameters.keys():
            for k2 in self._parameters[k1].keys():
                ...
                #print self._par_err
                #print k1,k2
                columns.append(pyfits.Column(name='%s_%s'%(k1,k2),format='E',array=[self._parameters[k1][k2]]))
                if self._par_err is not None:
                    #print self._par_err,k1,k2
                    columns.append(pyfits.Column(name='%s_%s_err'%(k1,k2),format='E',array=[self._parameters_err[k1][k2]]))
...
class Gaussians_flux(fit_profile1D):
    def _profile(self, x):
        ...
        ncomp = len(self._par)
        for i in range(ncomp):
            y += self._par[i]*numpy.exp(-0.5*((x-self._args[0])/self._args[1])**2)/(fact*self._args[1])
        return y
...
class Gauss_Hermite(fit_profile1D):
    def _profile(self, x):
        ...
        y = a*numpy.exp(-0.5*w**2)*(1. + h3*H3 + h4*H4+h5*H5+h6*H6+h7*H7+h8*H8)/(fact*sigma)
        return y
...
class Moffat(fit_profile1D):
    def _profile(self, x):
        ...
        r_d = fwhm /(2. *numpy.sqrt(2**(1. /beta) -1.))
        sigma_0 = (beta-1.)*flux/(numpy.pi*(r_d**2))
        y = sigma_0 *(1. + ((x-cent)/r_d)**2)**(-beta)     
        return y
...
class Gaussian2D(fit_profile2D):
    def _profile(self, x, y):
        return self._par[0]*numpy.exp(-0.5*(((x-self._par[1])/self._par[3])**2+((y-self._par[2])/self._par[4])**2))/(fact*fact*self._par[3]*self._par[4])
    ...
    def _guess_par(self, x, y, z):
        ...
        self._par[3]  = numpy.sqrt(numpy.sum(z[indrow]*(x[indrow]-self._par[2])**2)/numpy.sum(z[indrow]))
        self._par[4]  = numpy.sqrt(numpy.sum(z[indcol]*(y[indcol]-self._par[1])**2)/numpy.sum(z[indcol]))
...
def gaussian(p, x):
    return abs(p[0])*numpy.exp(-0.5*((x-p[1])/abs(p[2]))**2)/(fact*abs(p[2]))

def gaussian_const(p, x):
    return p[0]*numpy.exp(-0.5*((x-p[1])/p[2])**2)/(fact*p[2]) + p[3]

def gaussian2d(p, x, y):
    return p[0]*numpy.exp(-0.5*(((x-p[1])/p[3])**2+((y-p[2])/p[4])**2))/(fact*fact*p[3]*p[4])

def gaussian_poly(p, x):
    return p[0]*numpy.exp(-0.5*((x-p[1])/p[2])**2)/(fact*p[2]) + numpy.polyval(p[3:],x)

def gaussian_multi(p, x):
    y = numpy.zeros(len(x))
    ncomp = len(p)/3
    for i in range(ncomp):
        y += abs(p[i])*numpy.exp(-0.5*((x-p[i+ncomp])/abs(p[i+2*ncomp]))**2)/(fact*abs(p[i+2*ncomp]))
    return y
...
def gaussian_width_multi_offset(p, x, pos):
    ...
    ncomp = len(pos)
    for i in range(ncomp):
        y += p[i+1]*numpy.exp(-0.5*((x-pos[i]+p[-1])/p[0])**2)/(fact*p[0])
    return y
...
def gauss_hermite(p, x):#, a, mean, sigma, h3, h4):
    ...
    return a*numpy.exp(-0.5*w**2)*(1. + h3*H3 + h4*H4)/(fact*sigma)
