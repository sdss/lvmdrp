...
class PassBand(Spectrum1D):
	...
	def effectiveWave(self):
		...
		b=numpy.trapz(old_div(self._data,self._wave))
		eff_wave=numpy.sqrt(old_div(a,b))
	...
	def fluxToMag(self, flux, error=None, system='AB', units=1.0e-16, wave=None):
		if system=='AB':
			...
			effLMicron=self.effectiveWave()*(old_div(1e-10,1e-6))

			fluxJy=old_div((flux*units*effLMicron**2),aLambda)
			mag=-2.5*numpy.log10(old_div(fluxJy,1e23))-48.6
		if system=='Vega':
