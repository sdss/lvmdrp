...
class Image(Header):
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Image):
            ...
            if self._error is not None and other._error is not None:
                new_error = numpy.sqrt(self._error.astype(numpy.float64)**2+other._error.astype(numpy.float64)**2)
    ...
    def cutOverscan(self, bound_x, bound_y, subtract=True):
        ...
        idx = numpy.indices(self.getDim()) # generate the index array for the x and y positions
        # select data outside the cut out region (overscan)
        select = numpy.logical_and(numpy.logical_and(idx[0]>=int(bound_y[0])-1, idx[0]<=int(bound_y[1])-1), numpy.logical_and(idx[1]>=int(bound_x[0])-1, idx[1]<=int(bound_x[1])-1))
    ...
    def split(self,  fragments, axis='X'):
        ...
        for i in range(fragments) :
            image_list.append(Image(data=split_data[i], error=split_error[i],  mask=split_mask[i]))
    ...
    def medianImg(self, size, mode='nearest',use_mask=False):
        ...
        if self._mask is None and use_mask is True:
            ...
            new_data = ndimage.filters.generic_filter(self._data, numpy.nanmedian, size, mode=mode)
            new_mask = numpy.isnan(new_data)
            
        image = Image(data=new_data, header = self._header, error = self._error,  mask = new_mask) # create a new Image object
        return image
    ...
    def fitPoly(self, axis='y', order=4, plot=-1):
        ...
        for i in range(slices):
            ...
            if self._mask  is not None:
                if numpy.sum(valid[:,i])>order+5:
                    fit_par[:, i] = numpy.polyfit(x[valid[:,i]], self._data[valid[:,i],i], order) # fit polynom
                    res = numpy.polyval(fit_par[:,i], x[valid[:, i]])-self._data[valid[:, i], i] # compute residuals
                    ...
                    fit_par[:, i] = numpy.polyfit(x[valid[:,i]][select], self._data[valid[:, i],i][select], order) # fit #refit polynomial with clipped data
                    if plot==i:
                        pylab.plot(x, self._data[:, i], '-b')
                        pylab.plot(x[valid[:, i]][select], self._data[valid[:, i], i][select], 'ok')
        ...
        if self._mask is not None:
            ...
            new_mask = None
        new_img = Image(data=fit_result, error=self._error, header = self._header,  mask=new_mask)
        return new_img

    def traceFWHM(self, axis_select,  TraceMask, blocks, init_fwhm, threshold_flux,max_pix=None):
           # create an empty trace  of the given size
        fwhm = numpy.zeros((TraceMask._fibers, TraceMask._data.shape[1]), dtype=numpy.float32)
        ...
        for i in pixels[axis_select]:
         ...
         #   print(slice_img._data.shape, trace)
            fwhm_fit= slice_img.measureFWHMPeaks(trace, blocks, init_fwhm=init_fwhm, threshold_flux=threshold_flux, plot=plot) # obtain the fiber profile FWHM for each slice
            fwhm[good,i] = fwhm_fit[0]
            mask[good,i] = fwhm_fit[1]#    traceFWHM.setSlice(i, axis='y', data = fwhm_fit[0], mask = fwhm_fit[1]) # insert the result into the trace mask
            #return traceFWHM
        return (fwhm, mask)
    ...
    def extractSpecAperture(self, TraceMask, aperture):
        ...
        for i in range(self._dim[1]):
            ...
            pixels = numpy.round(pos[:, i][:, numpy.newaxis]+numpy.arange(-(aperture-1)/2.0, (aperture-1)/2.0+0.001, 1.0)[numpy.newaxis, :]).astype('int')
            bad= numpy.logical_or(numpy.sum(pixels>self._dim[0]-1, 1)>0,numpy.sum(pixels<2, 1)>0)
    ...
    def extractSpecOptimal(self, TraceMask, TraceFWHM, plot=-1):
        ...
        for i in range(self._dim[1]):
            ...
            trace=slice_trace[0]
            bad_fiber = numpy.logical_or((slice_trace[2]==1),numpy.logical_or(slice_trace[0]<0,slice_trace[0]>len(slice_img._data)-1))
            ...
            if i==plot:
                result = slice_img.obtainGaussFluxPeaks(trace[good_fiber], fwhm[good_fiber], indices, plot=True)
            else:
    ...
    def maskFiberTraces(self, TraceMask, aperture=3, parallel='auto'):
        ...
        if cpus>1:
            ...
            mask_pixels=[]
            for i in range(self._dim[1]):
                mask_pixels.append(pool.apply_async(numpy.unique, ([numpy.clip(numpy.round(trace[:, i][numpy.newaxis, :]+dx[:, numpy.newaxis]), 0, self._dim[0]-1).astype('int16')])))
            pool.close()
            pool.join()
    ...
    def peakPosition(self, guess_x=None, guess_y=None, box_x=None, box_y=None):
        ...
        if box_x is not None and box_y is not None:
            ...
            position=numpy.array(ndimage.maximum_position(image[min_y:max_y, min_x:max_x]))
    ...
    def centroidGauss(self, pix_x, pix_y, aperture=8, init_back=0.1, plot=False):
        ...
        if numpy.sum(spec_x._data)!=0 and numpy.sum(spec_y._data)!=0:
            out_x = spec_x.fitSepGauss([pix_x], aperture=aperture, init_back=init_back, plot=plot)
            cent_x = out_x[1]

            out_y = spec_y.fitSepGauss([pix_y], aperture=aperture, init_back=init_back, plot=False)
            cent_y = out_y[1]
        else:
            cent_x=0
            cent_y=0
    ...
    def centreBary(self, guess_x, guess_y, box_size, exponent=4):
        region = self._data[int(guess_y-box_size/2.0):int(guess_y+box_size/2.0), int(guess_x-box_size/2.0):int(guess_x+box_size/2.0)]
        ...
        if self._mask is not None:
            ...
            cent_x = numpy.sum((idx_x*region**exponent)[good_pix])/numpy.sum(region[good_pix]**exponent)+1
            cent_y = numpy.sum((idx_y*region**exponent)[good_pix])/numpy.sum(region[good_pix]**exponent)+1
        else:
            cent_x = numpy.sum((idx_x*region**exponent))/numpy.sum(region**exponent)+1
            cent_y = numpy.sum((idx_y*region**exponent))/numpy.sum(region**exponent)+1
        return cent_x, cent_y
    ...
    def centreFit(self,model='Gauss',cent_x=None, cent_y=None, box_size=None):
        ...
        if cent_x is not None and cent_y is not None and box_size is not None:
            if model=='Gauss':
                fit_model = models.Gaussian2D(amplitude=10.0,x_mean=float(box_size)/2.0,y_mean=float(box_size)/2.0,x_stddev=2.0,y_stddev=2.0)
            
            image_cut = self._data[int(cent_y-box_size/2.0):int(cent_y+box_size/2.0), int(cent_x-box_size/2.0):int(cent_x+box_size/2.0)]
            ...
            if model=='Gauss':
                fit_model = models.Gaussian2D(amplitude=10.0,x_mean=self._dim[1]/2.0,y_mean=self._dim[0]/2.0,x_stddev=2.0,y_stddev=2.0)
    ...
    def extractAper(self, cent_x, cent_y, aperture, kmax=1000, correct_masked=False, ignore_mask=True):
        ...
        def get_aperture_cover(i,j,xc,yc,r_ap,qrn_seq):
            ...
            area[select] = numpy.sum(numpy.sqrt(numpy.square(xx) + numpy.square(yy)) <= r_ap,0)/float(qrn_seq.shape[0])
            return area

        def integrate_aperture(image, xc, yc, r_ap, kmax, error=None, mask=None, ignore_mask=ignore_mask, correct_masked=correct_masked):
            dim = image.shape
    ...
    def extractApertures(self, posTab,  ref_pixel_x, ref_pixel_y, arc_scale, angle=0, offset_arc_x=0.0, offset_arc_y=0.0):
        ...
        offset_x=(offset_arc_x*numpy.cos(float(angle)/180.0*numpy.pi)-offset_arc_y*numpy.sin(float(angle)/180.0*numpy.pi))/arc_scale
        offset_y=(offset_arc_x*numpy.sin(float(angle)/180.0*numpy.pi)+offset_arc_y*numpy.cos(float(angle)/180.0*numpy.pi))/arc_scale
...
def loadImage(infile,  extension_data=None, extension_mask=None, extension_error=None, extension_header=0):
    ...
    image = Image()
    image.loadFitsData(infile,  extension_data=extension_data,  extension_mask=extension_mask, extension_error=extension_error, extension_header=extension_header)
