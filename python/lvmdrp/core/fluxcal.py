...
def djs_laxisnum(dimens, iaxis=None):
    ...
    if ndimen == 1:
        ...
        if iaxis == 0:
            for ii in range(0, naxis[0]-1): result[ii,:] = ii
        elif iaxis == 1:
            for ii in range(0, naxis[1]-1): result[:,ii] = ii
    elif ndimen == 3:
        if iaxis == 0:
            for ii in range(0, naxis[0]-1): result[ii,:,:] = ii
        elif iaxis == 1:
            ...
            for ii in range(0, naxis[2]-1): result[:,:,ii] = ii
    else:
...
def djs_reject(ydata, ymodel, outmask=None, inmask=None, sigma=None, invvar=None, upper=None, lower=None, maxdev=None, maxrej=None, groupsize=None, groupdim=None, sticky=None, groupbadpix=None, grow=None):
...
#    endif
    ...
    ndata = len(ydata)
    if (ndata == 0): print('No data points')
    if (ndata != len(ydata)): print('Dimensions of YDATA and YMODEL do not agree')

    if inmask is not None:
        if (ndata != len(inmask)): print('Dimensions of YDATA and INMASK do not agree')

    if maxrej is not None:
        if groupdim is not None:
            if (len(maxrej) != len(groupdim)): print('MAXREJ and GROUPDIM must have same number of elements!')
        if groupsize is not None:
            if (len(maxrej) != len(groupsize)): print('MAXREJ and GROUPSIZE must have same number of elements!')
        else:
            groupsize = ndata
   ...
   # Create OUTMASK, setting =1 for good data
    ...
    if outmask is not None:
        if (ndata != len(outmask)): print('Dimensions of YDATA and OUTMASK do not agree')
    else:
        outmask = np.ones_like(ydata, dtype=bool)

    if ymodel is None:
        if inmask is not None: outmask[:] = inmask
        return 0
    ...
    if (sigma is None and invvar is None):
        if inmask is not None: igood = np.where(inmask & outmask)[0]
        else: igood = np.where(outmask)
        ngood = igood.size
    ...
    if inmask is not None: badness = badness * inmask
    if sticky is not None: badness = badness * outmask
    ...
    if maxrej is not None:
        ...
        for iloop in range(0, (len(groupdim)>1)-1):
            ...
            for ivec in range(0, np.max(dimnum)):
                if dimnum is not None: indx = np.where(dimnum == ivec)[0]
                else: indx = np.linspace(ndata)
                ...
                if groupbadpix is not None:
                    ...
                    if groupsize is None:
                        ...
                        groups_upper = ((np.linspace(ngroups)+1) * groupsize < nin) - 1
    ...
    if inmask is not None: newmask = newmask & inmask
    if sticky is not None: newmask = newmask & outmask
...
def cholesky_band(lower, mininf=None, verbose=False):
    ...
    bi = np.linspace(kn)
    for i in range(1, kn-1): bi = [bi,np.linspace(kn-i)+(kn+1)*i]
...
def intrv(x, fullbkpt, nbkptord): 
    ...
    for i in range(0, nx-1):
        while (x[i] > fullbkpt[ileft+1] and ileft < n-1 ): ileft = ileft + 1
        indx[i] = ileft
...
def bsplvn(bkpt, nord, x, ileft):
    ...
    if nx > 12000000:
        ...
        while lower < nx:
            ...
            vnikx[lower:upper, :] = bsplvn(bkpt, nord, x[lower:upper], ileft[lower:upper])
            lower = upper + 1
...
def bspline_action(x, sset, x2=None):
    ...
    if x2 is not None:
        ...
        if "npoly" in sset.keys(): npoly = sset["npoly"]
 

    nord = sset["nord"]
    goodbk = np.where(sset["bkmask"] != 0)
    nbkpt = goodbk.size
    if nbkpt < 2*nord: return -2, lower, upper
    n = nbkpt - nord
    ...
    if x2 is not None:
        ...
        if sset["funcname"] == 'poly':
            ...
            for i in range(1,npoly-1): temppoly[:, i] = temppoly[:, i-1] * x2norm
...
def bspline_valu(x, sset, x2=None, action=None, upper=None, lower=None):
    ...
    for i in range(0, n - nord):
        ...
        if (ict > 0):
            yfit[lower[i]:upper[i]] = goodcoeff[i*npoly+spot] @ action[lower[i]:upper[i], :]
    ...
    if outside.size != 0: mask[outside] = 0

    hmm = np.where(goodbk[1:] - goodbk > 2)[0]
    nhmm = hmm.size 
    for jj in range(0, nhmm - 1):
        inside = np.where((x >= sset["fullbkpt"][goodbk[hmm[jj]]])&(x <= sset["fullbkpt"][goodbk[hmm[jj]+1]-1]))
        if inside.size != 0: mask[inside] = 0
...
def bspline_bkpts(x, nord, bkpt=None, bkspace=None, nbkpts=None, everyn=None, silent=True, bkspread=None, placed=None):
...
def bspline_fit(xdata, ydata, invvar, sset, fullbkpt=None, x2=None, npoly=None, nord=None):

    if nord is None: nord = 4

    if not isinstance(sset, dict): sset = create_bsplineset(fullbkpt, nord, npoly=npoly) 

    if "npoly" in sset: npoly = sset["npoly"]
    if npoly is None: npoly = 1
   ...
   #  The next line is REQUIRED to fill a1
    ...
    bi = np.linspace(bw)
    bo = np.linspace(bw)
    for i in range(1, bw-1): bi = [bi, np.linspace(bw-i)+(bw+1)*i]
    for i in range(1, bw-1): bo = [bo, np.linspace(bw-i)+bw*i]
    ...
    sc = len(sset["coeff"])
    if (sc[0] == 2):
        sset["icoeff"][:, goodbk] = np.reshape(alpha[0,np.linspace(nfull)], (npoly,nn))
...
def bspline_iterfit(xdata, ydata, invvar=None, nord=None, x2=None, npoly=None, xmin=None, xmax=None, bkpt=None, oldset=None, maxiter=None, upper=None, lower=None, requiren=None, fullbkpt=None, funcname=None, **kwargs):
   ...
   # Check dimensions of inputs
    ...
    if nord is None: nord = 4
    if upper is None: upper = 5
    if lower is None: lower = 5
    ...
    if oldset is not None:
        ...
        if fullbkpt is None:
            fullbkpt = bspline_bkpts(xdata[xsort[these]], nord=nord, bkpt=bkpt, **kwargs)
        ...
        if x2 is not None:
            if xmin is None: xmin = np.min(x2)
            if xmax is None: xmax = np.max(x2)
    ...
    if x2 is not None: x2work = x2[xsort]
    ...
    while (((error != 0) or (qdone == 0)) and iiter < maxiter):
        ...
        if (ngood < 1 or goodbk[0] != -1):
            ...
            if requiren is not None:
                ...
                for ileft in range(nord, ngb-nord):
                    while(xwork[i] >= sset["fullbkpt"][goodbk[ileft]] and xwork[i] < sset["fullbkpt"][goodbk[ileft+1]] and i < nx-1):
            ...
            error = bspline_fit(xwork, ywork, invwork*maskwork, sset, x2=x2work, yfit=yfit, nord=nord, **kwargs)
        ...
        if (error == -2):
            ...
            qdone = djs_reject(ywork, yfit, invvar=invwork, inmask=inmask, outmask=maskwork, upper=upper, lower=lower, **kwargs)
...
def typingmodule(objflux, loglam, objivar, dispimg, sfd_ebv, psfmag, plottitle="", template="kurucz", thekfile="", targetflag=None):
    ...
    unreddenfactor = 1 / f99_ext.extinguish(10.**loglam, Ebv=1.0)

    #   !p.multi = [0,1,2]
    modflux = np.zeros_like(objflux)
    # Find the best-fit model -- evaluated for each exposure [NPIX,NEXP]
    thismodel, kindx = spflux_bestmodel(loglam, objflux*unreddenfactor**sfd_ebv, objivar/unreddenfactor**(2*sfd_ebv), plottitle=plottitle, template=template)
    ...
    bside = np.where(tmploglam < np.log10(6300.)) 
    rside = np.where(tmploglam > np.log10(5900.))
    middle = np.where((tmploglam < np.log10(6300.)) & (tmploglam > np.log10(5900.)))
    ...
    tmpdispimg[middle] = (bluedispimg[middle] + reddispimg[middle])/2.

    #tmpflux = spflux_read_kurucz(tmploglam-np.log10(1+kindx.z), tmpdispimg,
    #    iselect=kindx.imodel)

    if template == 'kurucz':
        tmpflux = spflux_read_kurucz(tmploglam-np.log10(1+kindx.z), tmpdispimg, iselect=kindx.imodel, thekfile=thekfile)
    elif template == 'munari':
        tmpflux = spflux_read_munari(tmploglam-np.log10(1+kindx.z), tmpdispimg, iselect=kindx.imodel, thekfile=thekfile)
    elif template == 'BOSZ':
        tmpflux = spflux_read_bosz(tmploglam-np.log10(1+kindx.z), tmpdispimg, iselect=kindx.imodel, thekfile=thekfile)
    else:
    ...
    if targetflag and sdss_flagval('MANGA_TARGET2',['STELLIB_PS1','STD_PS1_COM']) != 0:
        photometry = 'ps1'
    ...
    if photometry == 'ps1':
        fthru = filter_thru(tmpflux * flambda2fnu, waveimg=wavevec, toair=True, filternames=['ps1_g.txt','ps1_r.txt','ps1_i.txt','ps1_z.txt'])
        thismag = -2.5*np.log10(fthru) - (48.6-2.5*17)
        thismag = np.asarray([[-999.],[thismag]])
        ...
        fthru = filter_thru(tmpflux * flambda2fnu, waveimg=wavevec, toair=True, filternames=['gaia_G.dat'])
        thismag = -2.5 * np.log10(fthru) - (48.6-2.5*17)
    elif photometry == 'gaiadr2':
        fthru = filter_thru(tmpflux * flambda2fnu, waveimg=wavevec, toair=True, filternames=['gaia_G.dat','gaia_BP.dat','gaia_RP.dat'])
        thismag = -2.5*np.log10(fthru) - (48.6-2.5*17)
        ...
        thismag = thismag + np.asarray([[25.6884-25.7916],[25.3514-25.3862],[24.7619-25.1162]])
        thismag = np.asarray([[-999.],[thismag],[-999.]])
    ...
    if photometry == 'gaiadr1':
        scalefac = 10.**((thismag-psfmag[1])/2.5)
        kindx.mag[1] = psfmag[1]
    elif photometry == 'gaiadr2':
        scalefac = 10.**((thismag[1]-psfmag[1])/2.5)
