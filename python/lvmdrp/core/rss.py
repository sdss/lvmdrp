	...
	def combineRSS(self, rss_in, method='mean', replace_error=1e10):
		...
		if method=='weighted_mean' and error is not None:
			if mask is not None:
				...
				good_pix = numpy.sum(numpy.logical_not(select), 0)
				select_mean = good_pix>0
				combined_data= old_div(numpy.sum(old_div(data,error**2), 0),numpy.sum(old_div(1,error**2), 0))
				combined_mask = good_pix==0
				combined_error = 1.0/numpy.sqrt(numpy.sum(old_div(1,error**2), 0))
	...
	def createCubeInterpolation(self, mode='inverseDistance', sigma=1.0, radius_limit=5, resolution=1.0, min_fibers=3, slope=2.0, bad_threshold=0.1, replace_error=1e10,store_cover=False):
		if self._shape=='C':
			...
			if mode=='inverseDistance':
				...
				weights_0[select] = numpy.exp(-0.5*(old_div(dist[select],sigma))**slope)

				for i in range(self._fibers):
		#            print i
					select = weights_0[:, :, i]>0
					select_bad = old_div(weights_0[:, :, i],numpy.sum(weights_0[:, :, i].flatten()))>bad_threshold
				...
				cube[select] = (old_div(cube[select],weights[select]))*resolution**2
				...
				for i in range(self._fibers):
	 #               print i
					aperture = Aperture(old_div((self._arc_position_x[i]-min_x),resolution), old_div((self._arc_position_y[i]-min_y),resolution), old_div(self._size[0],resolution))
					cover_fraction = old_div(aperture.cover_mask((dim_y, dim_x)),fiber_area)
		...
		if mode=='inverseDistance':
			...
			for j in range(self._fibers):
				...
				for i in range(self._res_elements):
					...
					select = dist<=radius_limit
					weights_0[i, select] = numpy.exp(-0.5*(old_div(dist[select],sigma))**slope)
					select_bad = old_div(weights_0[i, :, :],numpy.sum(weights_0[i, :, :].flatten()))>bad_threshold
		   ...
		   #     print j
				...
				for i in range(self._res_elements):
					...
					cover_fraction[i, :, :] = old_div(aperture.cover_mask((dim_y, dim_x)),fiber_area)
				#select_cos = cover_fraction>crit_cos
	...
	def createCubeInterDAR_new(self, offset_x, offset_y,min_x,max_x,min_y,max_y,dim_x,dim_y,mode='inverseDistance', sigma=1.0, radius_limit=5, resolution=1.0, min_fibers=3, slope=2.0, bad_threshold=0.1, full_field=False, replace_error=1e10, store_cover=False):
		if self._shape=='C':
			...
			if mode=='inverseDistance':
				...
				int_kernel = float(numpy.sum(numpy.exp(-0.5*(old_div(dist_test[select],sigma))**slope)))
				dim_x = int(dim_x)
				dim_y = int(dim_y)
				fibers = self._fibers
...
#            print numpy.arange(len(medians))[select_max]+1
	...
	def centreBary(self, guess_x, guess_y, radius, exponent=4):
		...
		select = dist<=radius
		bary_x = old_div(numpy.sum(self._data[select, :]**exponent*self._arc_position_x[select][:, numpy.newaxis], 0),numpy.sum(self._data[select, :]**exponent, 0))
		bary_y = old_div(numpy.sum(self._data[select, :]**exponent*self._arc_position_y[select][:, numpy.newaxis], 0),numpy.sum(self._data[select, :]**exponent, 0))
	...
	def registerImage(self, image, passband, search_box, step_search,  ref_pix_x, ref_pix_y, arc_scale, angle=0.0, guess_x=0.0,  guess_y=0.0, parallel='auto'):
		...
		for i in range(len(offsets_x)):
			for j in range(len(offsets_y)):
					offsets_xIFU[i, j] = old_div(offsets_x[i],arc_scale)
					offsets_yIFU[i, j] = old_div(offsets_y[j],arc_scale)
					if cpus==1:
						...
						flux = threads[m].get()
					good_pix = (old_div(flux[0],flux[1])>3.0)
					...
					match = numpy.median(old_div(flux[0][good][sort[:-1]],AB_flux[good][sort[:-1]]))
					...
					chisq[i, j] = float(numpy.sum(old_div((flux[0][good][sort[:-1]]-scale_flux[i, j]*AB_flux[good][sort[:-1]])**2,
