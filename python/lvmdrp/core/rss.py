...
class RSS(FiberRows):
    ...
    def from_spectra1d(cls, spectra_list, header=None, shape=None, size=None, arc_position_x=None, arc_position_y=None, good_fibers=None, fiber_type=None, logwave=False):
        ...
        if numpy.allclose(numpy.repeat(rss._wave[0][None, :], rss._fibers, axis=0), rss._wave):
            rss.setWave(rss._wave[0])
        if numpy.allclose(numpy.repeat(rss._inst_fwhm[0][None, :], rss._fibers, axis=0), rss._inst_fwhm):
    ...
    def createCubeInterpolation(self, mode='inverseDistance', sigma=1.0, radius_limit=5, resolution=1.0, min_fibers=3, slope=2.0, bad_threshold=0.1, replace_error=1e10,store_cover=False):
     ...
     #               print(i)
            ...
            dim_x = int(numpy.round(numpy.rint(float(max_x-min_x)/resolution), 4)+1)
            dim_y = int(numpy.round(numpy.rint(float(max_y-min_y)/resolution), 4)+1)
        ...
        if self._shape=='C':
            ...
            if mode=='inverseDistance':
               ...
               #     print(j)
                    cover_fraction[:, :, :]=0
                    for i in range(self._res_elements):
                        aperture = Aperture((self._arc_position_x[j]+offset_x[j,i]*resolution-min_x)/resolution, (self._arc_position_y[j]+offset_y[j,i]*resolution-min_y)/resolution, self._size[0]/resolution, grid_fixed=True)
                        cover_fraction[i, :, :] = aperture.cover_mask((dim_y, dim_x))/fiber_area
                    #select_cos = cover_fraction>crit_cos
                    #fiber_mask = numpy.logical_and(select_cos, (self._mask[i, :])[:, numpy.newaxis, numpy.newaxis])
                    mask = numpy.logical_or(mask,numpy.logical_and(cover_fraction>0, self._mask[j, :][:, numpy.newaxis, numpy.newaxis]))
                ...
                select2 = weights>0
                cube[select2] = cube[select2]/weights[select2]*(resolution**2)
                mask = numpy.logical_not(select2)#numpy.logical_or(mask, numpy.logical_not(select2))
                #cube[mask2]=0
                if self._error is not None:
                    error = numpy.sqrt(error)
                    corr_cube = numpy.sqrt(corr_cube)
    ...
    def createFiberFlat(self, smooth_poly=-5, smooth_median=0, clip=[0.2, 2], valid=None):
        ...
        if len(self._wave.shape)==1:
            if smooth_median>0:
                self._data = ndimage.filters.median_filter(self._data, (1, smooth_median))
...
def glueRSS(infiles, outfile):
    ...
    if len(hdrs)>0:
      ...
      hdr_out = None
    rss_out = RSS(wave=wave_out, data=data_out, error=error_out, mask=mask_out, inst_fwhm=fwhm_out, header=hdr_out.getHeader())
