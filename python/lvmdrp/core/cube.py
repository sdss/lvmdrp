...
class Cube(Header, PositionTable):
    def __getitem__(self, slice):
        ...
        if slice >= self._res_elements or slice < self._res_elements * -1:
            raise IndexError(
                "The Cube contains only %i resolution elments for which the index %i is invalid"
    ...
    def __mul__(self, other):
        if isinstance(other, Spectrum1D):
            ...
            data = (
                self._data
                * other._data.astype(numpy.float32)[:, numpy.newaxis, numpy.newaxis]
            )
            ...
            if self._error is not None and other._error is not None:
                error = numpy.sqrt(
                    (
                        self._error * other._data[:, numpy.newaxis, numpy.newaxis]
                        + self._data * other._error[:, numpy.newaxis, numpy.newaxis]
                    )
                    ** 2
                )
    ...
    def loadFitsData(
        ...
        if (
            ...
            and extension_errorweight is None
            and extension_cover is None
        ):
            ...
            if self._wave is None:
                ...
                try:
                    self._wave = (
                        numpy.arange(self._res_elements) - crpix
                    ) * self.getHdrValue("CDELT3") + self.getHdrValue("CRVAL3")
                except KeyError:
                    self._wave = (
                        numpy.arange(self._res_elements) - crpix
                    ) * self.getHdrValue("CD3_3") + self.getHdrValue("CRVAL3")
        else:
            self.setHeader(header=hdu[extension_hdr].header, origin=file)
    ...
    def writeFitsData(
        ...
        if (
            ...
            if extension_errorweight == 0:
                ...
                hdus[extension_errorweight] = pyfits.ImageHDU(
                    self._error_weight, name="ERRWEIGHT"
                )
    ...
    def collapseCube(
        self, mode="mean", start_wave=None, end_wave=None, computeError=False
    ):
        ...
        if mode == "mean":
            if numpy.sum(select_wave) > 0:
                ...
                if computeError and self._error is not None:
                    error_collapsed = numpy.sqrt(
                        numpy.sum(self._error[select_wave, :, :] ** 2, 0)
                    ) / numpy.sum(select_wave)
    ...
    def getAperSpec(
        ...
        spec = Spectrum1D(
            wave=self._wave, data=spec_data, mask=masked, error=spec_error
        )
        return spec

    def glueCubeSets(
        self, cube1, cube2, rescale_region=[], merge_wave=0.0, mergeHdr=True
    ):
        wave1 = cube1._wave
        wave2 = cube2._wave
        ...
        if merge_wave == 0.0:
            if (disp1) == (disp2) and (
                disp1 * numpy.rint(((wave2[0] - wave1[0])) / (disp1))
            ).astype(numpy.float32) == (wave2[0] - wave1[0]).astype(numpy.float32):
        ...
        self._data = numpy.zeros(
            (len(wave), yminus + yplus, xminus + xplus), dtype=numpy.float32
        )
        if cube1._error is not None and cube2._error is not None:
            self._error = numpy.zeros(
                (len(wave), yminus + yplus, xminus + xplus), dtype=numpy.float32
            )
        else:
            self._error = None
        ...
        if cube1._mask is not None and cube2._mask is not None:
            self._mask = numpy.zeros(
                (len(wave), yminus + yplus, xminus + xplus), dtype="bool"
            )
        else:
            self._mask = None

        if cube1._error_weight is not None and cube2._error_weight is not None:
            self._error_weight = numpy.zeros(
                (len(wave), yminus + yplus, xminus + xplus), dtype=numpy.float32
            )
        else:
            self._error_weight = None
        ...
        if rescale_region != []:
            ...
            slice1 = cube1.collapseCube(
                mode="median", start_wave=start_wave, end_wave=end_wave
            )
            slice2 = cube2.collapseCube(
                mode="median", start_wave=start_wave, end_wave=end_wave
            )
        ...
        for i in range(self._data.shape[2]):
            for j in range(self._data.shape[1]):
                if (
                    ...
                    ratio = (
                        slice2._data[j + crpix2_2 - crpix2, i + crpix1_2 - crpix1]
                        / slice1._data[j + crpix2_1 - crpix2, i + crpix1_1 - crpix1]
                    )
                else:
                    ratio = 1
                if ratio < 0:
                    ratio = 0
                if merge_wave != 0.0:
                    self._data[:, j, i] = numpy.concatenate(
                        (
                            cube1._data[
                                select1, j + crpix2_1 - crpix2, i + crpix1_1 - crpix1
                            ]
                            * ratio,
                            cube2._data[
                                select2, j + crpix2_2 - crpix2, i + crpix1_2 - crpix1
                            ],
                        )
                    )
                if self._error is not None:
                    self._error[:, j, i] = numpy.concatenate(
                        (
                            cube1._error[
                                select1, j + crpix2_1 - crpix2, i + crpix1_1 - crpix1
                            ]
                ...
                if self._error_weight is not None:
                    self._error_weight[:, j, i] = numpy.concatenate(
                        (
                            cube1._error_weight[
                                select1, j + crpix2_1 - crpix2, i + crpix1_1 - crpix1
                            ],
                ...
                if self._mask is not None:
                    if ratio > 0:
                        self._mask[:, j, i] = numpy.concatenate(
                            (
                                cube1._mask[
                                    select1,
                                    j + crpix2_1 - crpix2,
                ...
                if self._cover is not None:
                    self._cover[:, j, i] = numpy.concatenate(
                        (
                            cube1._cover[
                                select1, j + crpix2_1 - crpix2, i + crpix1_1 - crpix1
                            ],
...
def loadCube(
    ...
    cube.loadFitsData(
        ...
        extension_cover=extension_cover,
        extension_hdr=extension_hdr,
    )
