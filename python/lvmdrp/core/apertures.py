...
class Aperture(object):
    ...
    def cover(self, i, j):
        ...
        select = numpy.logical_and(
            rxy >= (self._radius - 0.71), rxy <= (self._radius + 0.71)
        )
        xx = (x[select] - 0.5)[numpy.newaxis, :] + self._sampeling[:, 1][
            :, numpy.newaxis
        ]
        yy = (y[select] - 0.5)[numpy.newaxis, :] + self._sampeling[:, 0][
            :, numpy.newaxis
        ]
        area[select] = numpy.sum(
            numpy.sqrt(xx**2 + yy**2) <= self._radius, 0
        ) / float(self._kmax)
        return area
    ...
    def integratedFlux(self, image):
        dim = image._dim
        xlim = [
            self._xCenter // 1 - 1 - math.ceil(self._radius) - 2,
            self._xCenter // 1 - 1 + math.ceil(self._radius) + 3,
        ]
        ylim = [
            self._yCenter // 1 - 1 - math.ceil(self._radius) - 2,
            self._yCenter // 1 - 1 + math.ceil(self._radius) + 3,
        ]
        ...
        if not xlim[0] >= xlim[1] and not ylim[0] >= ylim[1]:
            ...
            self._xCenter = self._xCenter - (
                self._xCenter + cor_x - 2 - math.ceil(self._radius)
            )
            self._yCenter = self._yCenter - (
                self._yCenter + cor_y - 2 - math.ceil(self._radius)
            )
...
class Apertures(object):
    def __mul__(self, other):
        newApertures = Apertures(
            self._xCenters * other,
            self._yCenters * other,
            self._radii * other,
            self._kmax,
        )
        return newApertures
    ...
    def offsetApertures(self, offset_x, offset_y):
        newApertures = Apertures(
            self._xCenters + offset_x,
            self._yCenters + offset_y,
            self._radii,
            self._kmax,
        )
        return newApertures
