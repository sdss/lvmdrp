...
def quick_reduction(expnum: int, use_fiducial_master: bool = False) -> None:
    ...
    for sci in sci_metadata.to_dict("records"):
        ...
        # interpolate & subtract sky fibers
        # sky_tasks.interpolate_sky(in_rss=wsci_path, out_sky=hskye_path, which="both", subtract=True, out_rss=wsci_path)
        ...
        sky_tasks.interpolate_sky(in_rss=wsci_path, out_sky=hskye_path, which="e")
        rss_tasks.resample_wavelength(in_rss=hskye_path, out_rss=hskye_path, method="linear", disp_pix=0.5, start_wave=iwave, end_wave=fwave, err_sim=10, parallel=0, extrapolate=False)
        rss_tasks.apply_fiberflat(in_rss=hskye_path, out_rss=hskye_path, in_flat=mflat_path)
        sky_tasks.interpolate_sky(in_rss=wsci_path, out_sky=hskyw_path, which="w")
        rss_tasks.resample_wavelength(in_rss=hskyw_path, out_rss=hskyw_path, method="linear", disp_pix=0.5, start_wave=iwave, end_wave=fwave, err_sim=10, parallel=0, extrapolate=False)
        rss_tasks.apply_fiberflat(in_rss=hskyw_path, out_rss=hskyw_path, in_flat=mflat_path)

        # quick sky subtraction
        sky_tasks.quick_sky_subtraction(in_rss=hsci_path, out_rss=hsci_path, in_skye=hskye_path, in_skyw=hskyw_path)
