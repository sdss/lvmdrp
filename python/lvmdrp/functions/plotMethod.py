...
def flexurePatternTarget_drp(trace_log, disp_log, figure, object, scale="200"):
    ...
    while i < len(disp_lines):
        ...
        if len(line) == 1:
            ...
            obj_disp.append(line[0].split(".")[0])
    ...
    if numpy.sum(select_obj) == 1:
        ...
        for j in range(len(obj_trace_line[select_obj][0])):
            select_line = (
                obj_disp_line[select_disp][0] == obj_trace_line[select_obj][0][j]
            )
            for x in range(len(obj_trace_x[select_obj][0][j])):
                ax.arrow(
                    obj_trace_x[select_obj][0][j][x],
                    obj_trace_y[select_obj][0][j][x],
                    obj_disp_offset[select_disp][0][select_line][0][x] * scale,
                    obj_trace_offset[select_obj][0][j][x] * scale,
                    width=5,
                    head_width=20,
                    head_length=10,
                    fc="k",
...
def flexureOffsetNight_drp(
    trace_log, disp_log, figure, hdrKey_x, hdrKey_y, hdrKey_El, hdrKey_Az
):
    """
    ...
    ax.text(-160, 1.6, r"$\sigma(\Delta x)=%.3f \mathrm{pix}$" % (std_xoff), fontsize=18)
    ax.text(-160, 1.3, r"$\sigma(\Delta y)=%.3f \mathrm{pix}$" % (std_yoff), fontsize=18)
    ax.set_xlim(-180, 180)
    ax.set_ylim(-1.9, 1.9)
