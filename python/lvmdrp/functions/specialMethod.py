...
def matchMasterTrace_drp(CALIB_trace, Master_trace, out_trace, split='CAL',poly_cross=-2,poly_disp='',start_pix='',end_pix=''):
    ...
    if poly_disp!='':
        for i in range(shift.shape[0]):
            spec = Spectrum1D(wave=numpy.arange(shift_cross.shape[1]), data=shift_cross[i, :])
            if i==-1:
                pylab.plot(spec._wave,spec._data,'ok')
            spec.smoothPoly(order=poly_cross, ref_base=numpy.arange(shift_cross.shape[1]),start_wave=1000)
            if i==-1:
                pylab.plot(spec._wave,spec._data,'-r')
                pylab.show()
            shift_cross[i,:] = spec._data
