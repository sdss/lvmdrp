...
def extinctCAVEX_drp(
    ...
    if len(cavex_lines) > 0:
        ...
        select_time = numpy.logical_and(
            numpy.logical_and(dates == date_min, UThours >= time_min),
            numpy.logical_and(dates == date_max, UThours <= time_max),
        )
        ...
        if numpy.sum(select_time) > 0:
            ...
            if UThour > 18.0 and UThour < 23.00:
                ...
                select_time = numpy.logical_or(
                    numpy.logical_and(dates == date_min, UThours > 14.0),
    ...
    header.setHdrValue(
        extinct_key, float("%.3f" % (out_av)), "V-band atmospheric extinction"
    )
    header.setHdrValue(
        extinct_key + "_SD",
        float("%.3f" % (std_av)),
        "std of V-band atmospheric extinction",
    )
    header.writeFitsHeader()


def matchMasterTrace_drp(
    CALIB_trace,
    Master_trace,
    out_trace,
    split="CAL",
    ...
    if split != "":
        ...
        master_calib = master_trc
    shift = calib_trc._data - master_calib._data
    shift_cross = numpy.zeros_like(shift)
    for i in range(shift.shape[1]):
        mask = master_calib._mask[:, i] == 0
        spec = Spectrum1D(wave=master_calib._data[mask, i], data=shift[mask, i])
        if i == -1:
        ...
        if i == -1:
            ...
            pylab.show()
        shift_cross[:, i] = spec._data
    if poly_disp != "":
        for i in range(shift.shape[0]):
            spec = Spectrum1D(
                wave=numpy.arange(shift_cross.shape[1]), data=shift_cross[i, :]
            )
            if i == -1:
                pylab.plot(spec._wave, spec._data, "ok")
            spec.smoothPoly(
                order=poly_cross,
                ref_base=numpy.arange(shift_cross.shape[1]),
                start_wave=1000,
            )
            if i == -1:
                pylab.plot(spec._wave, spec._data, "-r")
                pylab.show()
            shift_cross[i, :] = spec._data
    master_trc._data = master_trc._data + shift_cross
    master_trc.writeFitsData(out_trace)


def matchARCLamp_drp(
    arc_rss,
    arc_rss_ref,
    disp_ref,
    disp_out,
    ...
    for i in range(rss_disp._fibers):
        ...
        shift_spec.smoothPoly(
            order=poly_cross, ref_base=numpy.arange(arc._data.shape[1])
        )
...
def checkWavelengthRSS_drp(
    ...
    (fibers, flux, cent_wave, fwhm, masked) = rss.measureArcLines(
        ref_spec,
        line_list_pix,
        aperture=aperture,
        init_back=init_back,
    ...
    out_file.write(
        "#name input_wavelength median_wave mean_wave sigma_wave median_flux mean_flux sigma_flux  med_sigma mean_sigma sig_sigma\n"
    )
...
def matchSkySpecTime_drp(
    ...
    if err_sim > 1 and sky_1._error is not None:
        sky_out = Spectrum1D(
            wave=sky_wave,
            data=numpy.zeros(sky_1._dim, dtype=numpy.float32),
            error=numpy.zeros(sky_1._dim, dtype=numpy.float32),
        )
    else:
        sky_out = Spectrum1D(
            wave=sky_wave, data=numpy.zeros(sky_1._dim, dtype=numpy.float32)
        )
    ...
    sky_time[0] = (
        sky_1_header.getHdrValue(hdr_key_start)
        + (next_day[0] * 86400)
        + sky_1_header.getHdrValue(hdr_key_end)
        + (next_day[0] * 86400)
    ) / 2.0
    ...
    for i in range(1, len(sky_in)):
        ...
        sky_time[i] = (
            sky_header.getHdrValue(hdr_key_start)
            + (next_day[i] * 86400)
            + sky_header.getHdrValue(hdr_key_end)
            + (next_day[i] * 86400)
        ) / 2.0
    ...
    if hdr_obj.getHdrValue(hdr_key_end) < hdr_obj.getHdrValue(hdr_key_start):
        object_time = (
            numpy.arange(
                hdr_obj.getHdrValue(hdr_key_start),
                hdr_obj.getHdrValue(hdr_key_end) + 86400 + time_steps,
                time_steps,
            )
            - init_time
        )
        ...
        object_time = (
            numpy.arange(
                hdr_obj.getHdrValue(hdr_key_start),
                hdr_obj.getHdrValue(hdr_key_end) + time_steps,
                time_steps,
            )
            - init_time
        )
    ...
    if function == "exponential":
        for i in range(sky_1._dim):
            ...
            if err_sim > 1 and sky_1._error is not None:
                ...
                sky_out._error[i] = numpy.std(
                    exp_profile._par_err_models[:, 0][:, numpy.newaxis]
                    * numpy.exp(
                        object_time[numpy.newaxis, :]
                        / exp_profile._par_err_models[:, 1][:, numpy.newaxis]
                    )
                    + exp_profile._par_err_models[:, 2][:, numpy.newaxis]
                )
            else:
