...
def expandHdrKeys_drp(
    file, prefix, keywords="", exclude="", extension="0", verbose="0", removeEmpty="0"
):
    """
...
def copyHdr_drp(file_in, file_out, exclude="", extension="0", removeEmpty="1"):
    ...
    """
    hdr_in = Header()
    hdr_in.loadFitsHeader(
        file_in, extension=int(extension), removeEmpty=int(removeEmpty)
    )
    hdr_out = Header()
    hdr_out.loadFitsHeader(
        file_out, extension=int(extension), removeEmpty=int(removeEmpty)
    )
    keys = [
        "SIMPLE",
        "BITPIX",
...
def mergeHdr_drp(files_in, file_out, exclude="", extension="0", removeEmpty="0"):
    ...
    for i in range(len(files)):
        hdrs.append(Header())
        hdrs[i].loadFitsHeader(
            files[i], extension=int(extension), removeEmpty=int(removeEmpty)
        )
...
def addHvelcorHdr_drp(
    file,
    key,
    RAKey="RA",
    RAUnit="h",
    DECKey="DEC",
    ObsLongKey="CAHA TEL GEOLON",
    LongSignFlip=1,
    ObsLatKey="CAHA TEL GEOLAT",
    ObsAltKey="CAHA TEL GEOELEV",
    ModJulKey="MJD-OBS",
    extension="0",
):
    """
    ...
    hdr.setHdrValue(
        key, float("%.2f" % (vel_correction[0])), "Heliocentric vel correction [km/s]"
    )
    hdr.writeFitsHeader()


def addAstrometry_drp(
    file,
    ref_RA,
    ref_DEC,
    resolution_x,
    ...
    hdr.setHdrValue(
        "CD1_1", resolution_x / 3600.0 * -1 * numpy.cos(rotation / 180 * numpy.pi)
    )
    hdr.setHdrValue(
        "CD1_2", resolution_y / 3600.0 * -1 * numpy.sin(rotation / 180 * numpy.pi)
    )
    hdr.setHdrValue(
        "CD2_1", resolution_x / 3600.0 * -1 * numpy.sin(rotation / 180 * numpy.pi)
    )
    hdr.setHdrValue(
        "CD2_2", resolution_y / 3600.0 * numpy.cos(rotation / 180 * numpy.pi)
    )
    if wcsaxes == 3:
