...
def installESOSky_drp(reinstall_skycorr: bool = False, reinstall_skymodel: bool = False):
    ...
    if os.path.isdir(SKYCORR_INST_PATH):
        ...
        if out.returncode == 0:
            log.info("found existing skycorr installation")
    ...
    if not skycorr_installed or reinstall_skycorr:
        ...
        while skycorr_installer.isalive():
            ...
            if i == 0:
                log.info("waiting for skycorr installation to finish")
            time.sleep(0.5)
            i += 1
        ...
        if skycorr_installer.exitstatus == 0:
            ...
            log.error("error while installing skycorr")
    ...
    if os.path.isdir(SKYMODEL_INST_PATH):
        ...
        log.info("found existing installation of skymodel")
...
def quick_sky_subtraction(in_rss: str, out_rss, in_skye: str, in_skyw: str) -> RSS:
    ...
    ra_e, dec_e = rss._header["SKYERA"], rss._header["SKYEDEC"]
    ra_w, dec_w = rss._header["SKYWRA"], rss._header["SKYWDEC"]
    ra_s, dec_s = rss._header["SCIRA"], rss._header["SCIDEC"]
