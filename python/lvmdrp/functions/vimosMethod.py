...
def combineTwilight_drp(night, numbers):
    ...
    for i in range(len(list)):
        if i == 0:
            os.system(
                "ls VIMOS_IFU_TwFlats%03d_%04d_B.1.* > combine_LAMP.B1"
                % (night, int(list[i]))
            )
            os.system(
                "ls VIMOS_IFU_TwFlats%03d_%04d_A.2.* > combine_LAMP.A2"
                % (night, int(list[i]))
            )
            os.system(
                "ls VIMOS_IFU_TwFlats%03d_%04d_A.3.* > combine_LAMP.A3"
                % (night, int(list[i]))
            )
            os.system(
                "ls VIMOS_IFU_TwFlats%03d_%04d_B.4.* > combine_LAMP.B4"
                % (night, int(list[i]))
            )
        else:
            os.system(
...
def prepareCalib_drp(
    ...
    if fiberflat_wave == 1:
        ...
        rss.determine_wavelength_solution(
            ...
            flux_min="20.0",
            aperture="10",
            fwhm_max=str(fwhm_max),
            fiberflat="%.2f,%.2f,%.2f,LAMP_%s.disp_cor"
            % (start_wave, end_wave, disp_wave, chip),
            rel_flux_limits="0.3,4.0",
            verbose="1",
        )
    else:
        im.extract_spectra(
...
def prepareObject_drp(
    ...
    head.addHvelcorHdr_drp(
        ...
        RAUnit="d",
        DECKey="DEC",
        ObsLongKey="ESO TEL GEOLON",
        LongSignFlip=1,
        ObsLatKey="ESO TEL GEOLAT",
        ObsAltKey="ESO TEL GEOELEV",
        ModJulKey="MJD-OBS",
        extension="0",
    )
    if correct_HVEL:
    ...
    if sky_line_list != "":
        ...
        rss.correctPixTable_drp(
            ...
            "%s_%s.pix_tab.fits" % (name_obj, chip),
            smooth_poly_cross="1",
            smooth_poly_disp=flexure_order,
            poly_disp="5",
        )
        ...
        for file_name in dir:
            if (
                ...
                im.subtractBias_drp(
                    ...
                    boundary_x=boundaries_x[i],
                    boundary_y=boundaries_y[i],
                    gain="ESO DET OUT1 CONAD",
                    rdnoise="ESO DET OUT1 RON",
                )
            ...
            result.append(
                pool.apply_async(
                    ...
                    args=(
                        ...
                        "%s/ref_lines_ARC_VIMOS_MR_%s.txt" % (vimos_calib, chips[i]),
                        wave_start,
                        wave_end,
                        wave_disp,
                        15,
...
def reduceObjectMR_drp(
    ...
    if telluric_cor == 1:
        flux.correctTelluric_drp(
            ...
            airmass="CCD1 ESO TEL AIRM START",
        )


def reduceObjectHR_drp(
    night,
    name_obj,
    object_nr,
    wave_start=4880,
    ...
    for i in range(len(chips)):
        ...
        if flux_calib == 1:
            ...
            os.system(
                "%s_%s.disp_cor.fits %s_%s.fobj.fits"
...
def reduceStdMR_drp(
    ...
    for i in range(len(chips)):
        ...
        if ref_star != "":
            ...
            if mask_telluric != "":
                os.system(
                    "cp telluric_spec.fits telluric_spec_%s.fits"
                    % (chips[i].split(".")[1])
                )
                std_telluric.append(Spectrum1D())
                std_telluric[i].loadFitsData(
                    "telluric_spec_%s.fits" % (chips[i].split(".")[1])
                )
    ...
    for i in range(len(chips)):
        ...
        if ref_star != "":
            flux.createSensFunction_drp(
                ...
                coadd=200,
                extinct_curve="Paranal",
                out_star="star_%s.txt" % (chips[i].split(".")[1]),
                mask_wave=mask_wave,
                mask_telluric=mask_telluric,
