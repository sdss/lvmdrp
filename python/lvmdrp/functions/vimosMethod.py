...
def prepareCalib_drp(night, chip, boundary_x,boundary_y, peaks_ref, ARC_ref, start_wave, end_wave, disp_wave,fwhm_max, setup='', border='4', fiberflat='1',  CCD_mask='',parallel='1'):
	night=int(night)
	...
	tracePeaks_drp('LAMP_%s.sub.fits'%(chip),  'peaks_%s.txt'%(chip), 'tjunk_%s.trc.fits'%(chip), disp_axis='y', poly_disp='-5',steps=50,  threshold_peak=50, coadd=60, verbose=0)
	...
	detWaveSolution_drp('WAVE_%s.ms.fits'%(chip),  'WAVE_%s'%(chip),  ARC_ref,  poly_dispersion='-5',  poly_fwhm='-3,-5',  flux_min='20.0',  aperture='23', fwhm_max=str(fwhm_max),  rel_flux_limits='0.1,6.0',  verbose='0')
	...
	if fiberflat==1:
		...
		resampleWave_drp('LAMP_%s.rss.fits'%(chip),  'LAMP_%s.disp_cor.fits'%(chip),  start_wave=start_wave,  end_wave=end_wave,  disp_pix=disp_wave,  err_sim=0,  parallel=parallel)
...
def prepareObject_drp(name_obj, night, object, chip, boundary_x, boundary_y, sky_line_list, start_wave, end_wave, disp_wave, resolution_fwhm, flexure_order, CCD_mask='',correct_HVEL=False, straylight=True, parallel='1'):
	...
	if sky_line_list!='':
		...
		correctPixTable_drp('%s_%s.pix_tab.fits'%(name_obj, chip),  '%s_%s.pix_tab.fits'%(name_obj, chip), 'offsetWave_%s.log'%(chip),  '%s_%s.pix_tab.fits'%(name_obj, chip),  smooth_poly_cross='1',  smooth_poly_disp=flexure_order,  poly_disp='7')
	if float(resolution_fwhm)!=0.0:
		...
		resampleWave_drp('%s_%s.res.fits'%(name_obj, chip), '%s_%s.disp_cor.fits'%(name_obj, chip), start_wave=start_wave,  end_wave=end_wave,  disp_pix=disp_wave,  err_sim='500', correctHvel=HVEL_key, parallel=parallel)
	else:
		resampleWave_drp('%s_%s.pix_tab.fits'%(name_obj, chip), '%s_%s.disp_cor.fits'%(name_obj, chip), start_wave=start_wave,  end_wave=end_wave,  disp_pix=disp_wave,  err_sim='500', correctHvel=HVEL_key, parallel=parallel)
	includePosTab_drp('%s_%s.disp_cor.fits'%(name_obj, chip), '%s/vimos_HR_%s_pt.txt'%(vimos_calib,chip))

def reduceCalibMR_drp(night, fiberflat='1', wave_start=4880, wave_end=9300, wave_disp=2.0, parallel='auto'):
	...
	if cpus>1:
		...
		result=[]
		for i in range(len(chips)):
			result.append(pool.apply_async(prepareCalib_drp, args=(night,chips[i],boundaries_x[i],boundaries_y[i],'%s/master_VIMOS_%s_%s'%(vimos_calib,chips[i], peaks_guess), '%s/ref_lines_ARC_VIMOS_MR_%s.txt'%(vimos_calib, chips[i]), wave_start, wave_end,2.0,15, '', border, fiberflat, CCD_mask,1)))
		pool.close()
		pool.join()
	else:
		for i in range(len(chips)):
			prepareCalib_drp(night,chips[i],boundaries_x[i],boundaries_y[i],'%s/master_VIMOS_%s_%s'%(vimos_calib,chips[i], peaks_guess), '%s/ref_lines_ARC_VIMOS_MR_%s.txt'%(vimos_calib, chips[i]), wave_start, wave_end, wave_disp, 15, '', border, fiberflat,CCD_mask,'auto')
...
def reduceCalibHR_drp(night, fiberflat='1', wave_start=4880, wave_end=9300, wave_disp=2.0, setup='orange', except_chip='',parallel='auto'):
	chips=['B.1', 'A.2', 'A.3', 'B.4']
	if except_chip!='':
	...
	if cpus>1:
		...
		result=[]
		for i in range(len(chips)):
			result.append(pool.apply_async(prepareCalib_drp, args=(night,chips[i],boundaries_x[i],boundaries_y[i],'%s/master_VIMOS_%s_%s'%(vimos_calib,chips[i], peaks_guess), '%s/ref_lines_ARC_VIMOS_HR%s_%s%s'%(vimos_calib, set, chips[i], wave_guess), wave_start, wave_end, wave_disp,10.0, setup,  border, fiberflat, CCD_mask, 1)))
		pool.close()
		pool.join()
	else:
		for i in range(len(chips)):

			prepareCalib_drp(night,chips[i],boundaries_x[i],boundaries_y[i],'%s/master_VIMOS_%s_%s'%(vimos_calib,chips[i], peaks_guess), '%s/ref_lines_ARC_VIMOS_HR%s_%s%s'%(vimos_calib, set,  chips[i], wave_guess), wave_start, wave_end, wave_disp, 8, setup, border, fiberflat, CCD_mask, 'auto')
...
def reduceObjectHR_drp(night, name_obj, object_nr, wave_start=4880, wave_end=8500, wave_disp=2.0, res_fwhm=7.5, A_V=0.15, fiberflat='1', flux_calib='1', telluric_cor='1',flexure_correct='1',straylight='1',setup='orange', correct_HVEL=False, except_chip='', parallel='auto' ):
	chips=['B.1', 'A.2', 'A.3', 'B.4']
	if except_chip!='':
		except_chips = except_chip.split(',')
		for i in range(len(except_chips)):
	...
	mergeRSS_drp(merge_obj[:-1], '%s.disp_cor.fits'%(name_obj))
	...
	if flux_calib==1:
		if fiberflat==1:
			fluxCalibration_drp('%s.flat.fits'%(name_obj), '%s.fobj.fits'%(name_obj), 'ratio.txt', 'ESO TEL AIRM START', 'EXPTIME', extinct_curve='Paranal', ref_units='1e-16', target_units='1e-16', norm_sb_fib='')
		else:
			fluxCalibration_drp('%s.disp_cor.fits'%(name_obj), '%s.fobj.fits'%(name_obj), 'ratio.txt', 'ESO TEL AIRM START', 'EXPTIME', extinct_curve='Paranal', ref_units='1e-16', target_units='1e-16', norm_sb_fib='')
	if telluric_cor==1:
...
def reduceStdHR_drp(night, std_nr, wave_start='4880', wave_end='9300', wave_disp='2.0', res_fwhm='0.0', ref_star='', A_V=0.15, mask_wave='', mask_telluric='', smooth_poly=-12, except_chip='', straylight='1', parallel='auto'):
	chips=['B.1', 'A.2', 'A.3', 'B.4']
...
def subtractSkyField_drp(object_in,  object_out, sky_field, factor,  scale_region='',scale_ind=1, nsky=200):
	...
	for i in range(4):
		constructSkySpec_drp('sky_QD%d.fits'%(i+1), 'sky_spec_QD%d.fits'%(i+1),  clip_sigma=0.0, filter=vimos_calib+'R_Johnson.txt,0,1',nsky=nsky)
