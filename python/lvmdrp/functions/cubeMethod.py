...
def measureDARPeak_drp(
    ...
    cent_guess = collapsed_img.centreMax(
        ...
        cent_y=collapsed_img._dim[0] / 2.0,
        box_size=search_box,
    )
    ...
    for i in slices:
        if verbose == 1:
            print(i)
        image = cube[i]
        cent = image.centreBary(
            cent_guess[0] - 1, cent_guess[1] - 1, box_size=bary_box, exponent=bary_exp
        )
        cent_x[m] = cent[0]
        cent_y[m] = cent[1]
        m += 1
    ...
    select2 = numpy.logical_and(
        numpy.logical_and(diff_y > -1 * clip, diff_y < clip),
        numpy.logical_and(diff_x > -1 * clip, diff_x < clip),
    )
    ...
    rss_x = RSS(
        wave=cube._wave,
        data=numpy.ones(((fibers), len(cube._wave))) * spec2_x._data[numpy.newaxis, :],
        header=cube._header,
    )
    rss_x._data = rss_x._data.astype(numpy.float32) * res_cube

    rss_y = RSS(
        wave=cube._wave,
        data=numpy.ones(((fibers), len(cube._wave))) * spec2_y._data[numpy.newaxis, :],
        header=cube._header,
    )
    rss_y._data = rss_y._data.astype(numpy.float32) * res_cube
...
def fitDARPeak_drp(
    ...
    """
    The target position is measured by means of fitting a 2D Gaussian or Moffat to the image a long the wavelength to measure the effect of the DAR.
    The measured positions are smooth by a polynomial fit and clip to reject outliners due to night sky or object emission lines before a final
    polynomial fit is performed. The fitted positions are along the entire wavelength of the cube are stored in two FITS files in both, x and y, direction
    seperatly.
    ...
    for i in slices:
        if verbose == 1:
            print(i)
        image = cube[i]
        cent = image.centreFit("Gauss", cent_guess[0], cent_guess[1], box_size=box_size)
        # cent = image.centreFit('Gauss')
        # print(cent_guess)
        cent_x[m] = cent[0]
        cent_y[m] = cent[1]
        m += 1
...
def aperFluxCube_drp(
    ...
    try:
        ...
        cent_y = float(cent_y)
    spec = cube.getAperSpec(
        cent_x, cent_y, aperture, kmax=kmax, ignore_mask=True, correct_masked=False
    )
...
def matchAbsFluxAper_drp(cube_in, cube_out, key_mags, ref_mags, hdrkey, comment):
    ...
    for n in range(len(split_mags)):
        ...
        cube.setHdrValue(
            hdrkey + " RATIO " + ref_names[n],
            10 ** ((ref_mags[n] - mags[n]) / -2.5),
            comment,
        )
...
def matchCubeAperSpec_drp(
    ...
    select_wave = numpy.logical_and(
        cube1._wave > cube2._wave[3], cube1._wave < cube1._wave[-3]
    )
...
def subCubeWave_drp(cube_in, cube_out, wave_start=None, wave_end=None):
    ...
    cube_new = Cube(
        wave=cube_in._wave[select],
        data=cube_in._data[select, :, :],
        error=error,
        mask=mask,
        header=cube_in.getHeader(),
    )
    cube_new.writeFitsData(cube_out)


def glueCubesWavelength_drp(
    cube1_in,
    cube2_in,
    cube_out,
    rescale_region="",
    merge_wavelength="0.0",
    mergeHdr="1",
):
    ...
    cube.glueCubeSets(
        cube1,
        cube2,
        rescale_region=rescale_region,
        merge_wave=merge_wavelength,
        mergeHdr=mergeHdr,
    )
    cube.writeFitsData(cube_out)
...
def createSensFunction_drp(
    ...
    ref_star_resamp = ref_star_spec.resampleSpec(star_spec._wave, method="spline")
    if out_star != "":
    ...
    if mask_wave is not None:
        ...
        for i in range(regions):
            select = numpy.logical_and(
                sens_func._wave > mask_wave[i * 2],
                sens_func._wave < mask_wave[i * 2 + 1],
            )
    ...
    if verbose == 1:
        pylab.plot(
            sens_func._wave[good_pix][10:-10], sens_func._data[good_pix][10:-10], "-k"
        )
    if split == "":
        mask = sens_func._mask
        mask[:10] = True
        mask[-10:] = True
        sens_func_smooth = Spectrum1D(
            wave=sens_func._wave, data=sens_func._data, mask=mask
        )
        ...
        mask[-10:] = True
        sens_func_smooth2 = Spectrum1D(
            wave=sens_func._wave[select], data=sens_func._data[select], mask=mask
        )
    ...
    if split == "":
        ...
        min = numpy.argmin(
            numpy.abs(
                sens_func_smooth1._data[sens_func_smooth1._wave > split]
                - sens_func_smooth2._data[sens_func_smooth2._wave < split + overlap]
            )
        )
        #    print(min)
        ...
        for i in range(sens_func._dim):
            if i < change:
                ...
                out.write(
                    ...
                    % (
                        i,
                        sens_func_smooth2._wave[i - start2],
                        sens_func_smooth2._data[i - start2],
                    )
                )
...
def combineCubes_drp(incubes, outcube, method="mean", replace_error="1e10"):
    ...
    if method == "mean":
        image = numpy.zeros(
            (len(incubes), data.shape[1], data.shape[2]), dtype=numpy.float32
        )
        image2 = numpy.zeros(
            (len(incubes), data.shape[1], data.shape[2]), dtype=numpy.float32
        )
        error_img = numpy.zeros(
            (len(incubes), data.shape[1], data.shape[2]), dtype=numpy.float32
        )
        mask_img = numpy.zeros(
            (len(incubes), data.shape[1], data.shape[2]), dtype="bool"
        )
        for i in range(data.shape[0]):
            ...
            data[i, select_good] = (
                numpy.sum(image, 0)[select_good] / good_pix[select_good]
            )
...
def createTelluricCorrection_drp(cube_in, telluric_out, airmass, mask_telluric):
    cube = loadCube(cube_in)
    img = cube.collapseCube(mode="median")
    peakPosition = img.peakPosition()
    star_telluric1 = cube.getAperSpec(peakPosition[1], peakPosition[0], 4)
    star_telluric2 = cube.getAperSpec(peakPosition[1], peakPosition[0], 4)

    mask_telluric = numpy.array(mask_telluric.split(",")).astype("float32")
    airmass = float(airmass)
    regions = len(mask_telluric) / 2
    for i in range(regions):
        select_region = numpy.logical_and(
            star_telluric1._wave > mask_telluric[i * 2],
            star_telluric1._wave < mask_telluric[i * 2 + 1],
        )
        select_blue = numpy.logical_and(
            star_telluric1._wave > mask_telluric[i * 2] - 20,
            star_telluric1._wave < mask_telluric[i * 2],
        )
        select_red = numpy.logical_and(
            star_telluric1._wave > mask_telluric[i * 2 + 1],
        ...
        telluric_spec.writeFitsData(telluric_out)


def fitCubeELines_drp(
    cube_in,
    par_file,
    maps_prefix,
    smooth_box="",
    ...
    wave = cube._wave
    out_model = Cube(
        wave=wave, data=numpy.zeros_like(cube._data), header=cube.getHeader()
    )
    out_residual = Cube(
        wave=wave, data=numpy.zeros_like(cube._data), header=cube.getHeader()
    )
    ...
    if cube._error is not None:
        ...
        cube._error = numpy.ones_like(cube._data)
    if cube._mask is None:
        cube._mask = numpy.zeros(cube._data.shape, dtype="bool")
    ...
    if wave_range != "":
        ...
        for i in range(len(lines)):
            line = lines[i].split()
            if len(line) == 2:
                select_wave = numpy.logical_or(
                    numpy.logical_and(
                        cube._wave >= float(line[0]), cube._wave <= float(line[1])
                    ),
                    select_wave,
                )
    else:
    ...
    for n in par._names:
        ...
        if par._profile_type[n] == "Gauss":
            if err_sim > 0 and cube._error is not None:
                model["flux"] = Image(
                    data=numpy.zeros((cube._dim_y, cube._dim_x), dtype=numpy.float32),
                    error=numpy.zeros((cube._dim_y, cube._dim_x), dtype=numpy.float32),
                )
                model["vel"] = Image(
                    data=numpy.zeros((cube._dim_y, cube._dim_x), dtype=numpy.float32),
                    error=numpy.zeros((cube._dim_y, cube._dim_x), dtype=numpy.float32),
    ...
    for i in range(cube._dim_y):
        for j in range(cube._dim_x):
            sys.stdout.write(
                "Processing... %.1f/100%%\r"
                % (z / float(cube._dim_x * cube._dim_y) * 100)
            )
            sys.stdout.flush()
            # print(i, j)
            if numpy.sum(cube._data[:, i, j] != 0) > 0:
                ...
                if smooth_box != "":
                    ...
                    spec_fit = Spectrum1D(
                        wave=wave[select_wave],
                        data=spec._data[select_wave],
                        error=spec._error[select_wave],
                        mask=spec._mask[select_wave],
                    )
                else:
                    spec_fit = Spectrum1D(
                        wave=wave[select_wave],
                        data=cube._data[select_wave, i, j],
                        error=cube._error[select_wave, i, j],
                        mask=cube._mask[select_wave, i, j],
                    )
                if guess_window != "":
                ...
                out_residual._data[:, i, j] = (
                    cube._data[:, i, j] - out_model._data[:, i, j]
                )
                # print(i, j, 'done')
                for n in fit_par._names:
                    if fit_par._profile_type[n] == "Gauss":
                        ...
                        maps[n]["disp"]._data[i, j] = (
                            numpy.fabs(fit_par._parameters[n]["disp"]) * 2.354
                        )
                        if err_sim > 0 and cube._error is not None:
                            maps[n]["flux"]._error[i, j] = fit_par._parameters_err[n][
                                "flux"
                            ]
                            maps[n]["vel"]._error[i, j] = fit_par._parameters_err[n][
                                "vel"
                            ]
                            maps[n]["disp"]._error[i, j] = (
                                numpy.fabs(fit_par._parameters_err[n]["disp"]) * 2.354
                            )
                    elif fit_par._profile_type[n] == "TemplateScale":
                        maps[n]["scale"]._data[i, j] = fit_par._parameters[n]["scale"]
                        if err_sim > 0 and cube._error is not None:
                            maps[n]["scale"]._error[i, j] = fit_par._parameters_err[n][
                                "scale"
                            ]
            z += 1
    sys.stdout.write("\n")
    ...
    for n in par._names:
        if fit_par._profile_type[n] == "Gauss":
            ...
            maps[n]["scale"].writeFitsData(
                maps_prefix + "_Template_" + n + "_scale.fits"
            )
