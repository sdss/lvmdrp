...
def createParFile(
    ...
    install=skycorrinstall,
    defaultError=0.01,
    wave2Micron=1.0,
    vacOrAir="vac",
    dateKey="MJD-OBS",
    timeKey="TM-START",
    telAltKey="ESO TEL ALT",
    timeVal=None,
    dateVal=None,
    telAltVal=None,
    linetab="airglow_groups.dat",
    vardat="airglow_var.dat",
    ...
    if ncolnames > 4:
        raise ValueError("Colnames must have no more than 4 elements")
    if ncolnames < 4:
        nnone = 4 - ncolnames
...
def asciiSkyCorrWrapper(
    ...
    if cleanAqua:
        ...
        sp.call(
            [
                "ps aux | grep AquaTerm.app | grep -v grep | awk '{print $2}' | xargs kill"
            ],
            shell=True,
            stdout=FNULL,
            stderr=FNULL,
...
def readWaveInfo(dir, resFile, verbose=False):
    ...
    if len(info) == 0:
        if verbose:
            print("WARNING: no wave info found!")
        coefs = None
    else:
...
def test1():
    ...
    print(installdir)
    parfile = createParFile(
        installdir + "examples/data/sky_sinfo_1.fits",
...
def test2(
    objfile="examples/data/sky_sinfo_1.fits",
    skyfile="examples/data/sky_sinfo_2.fits",
