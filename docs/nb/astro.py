...
def distance(r1, d1, r2, d2):
    ...
    r2 = r2 / RADIAN
    d2 = d2 / RADIAN
    xlambda = math.sin(d1) * math.sin(d2) + math.cos(d1) * math.cos(d2) * math.cos(
        r1 - r2
    )
...
def get_pixel_scale(imagename="foo.fits", x=100, y=100):
    ...
    """
    print(
        "Get_pixel_scale has been stubbed out for now in the move to astroconda. If needed it should be reimplemented"
    )
    # ix=10
...
def radec2xy(imagename="foo.fits", ra=30.2, dec=35.0):
    """
